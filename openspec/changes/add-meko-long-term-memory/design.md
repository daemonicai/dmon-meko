...
`ILongTermMemory(Meko)` is an MCP client to `https://mcp.mekodata.ai/mcp` (Streamable HTTP, `mko_tkn_` bearer). Tool→method mapping: `AddFactAsync`→`memory_add(text)`; `RecordAsync`→`memory_add(messages)` (capture-gated); `SearchAsync`→`memory_search`; `GetAsync`→`memory_get_by_id`; `ListAsync`→`memory_get_all`; `UpdateAsync`→`memory_update`; `DeleteAsync`→`memory_delete_by_id`. **`FlushAsync` does NOT map to `flush_pending_memory_candidates`** — live verification (2026-05-30) showed that tool performs no server write and only returns an agent-directive (and it rejects `run_id`), so `FlushAsync` for this tier is a best-effort no-op (D8 / consistency requirement).
...
- **`flush_pending_memory_candidates` is an agent-directive, not a server barrier — VERIFIED live (2026-05-30).** Its description confirms *"This tool performs no database write itself"*; it returns a checklist for a hook-less agent to scan turns and call `memory_add`, and it does not accept `run_id`. Resolution: dmon captures explicitly at `RecordAsync`, so there is nothing to flush; `FlushAsync(Meko)` is a best-effort **no-op** and does not call the directive tool. Acting on the directive would be an agent-loop concern, out of scope for the store.
