...
- **M.E.AI ABI** → `ModelContextProtocol` depends on `Microsoft.Extensions.AI.Abstractions`; a version drift from dmon-core's 10.5.1 pin risks the ABI mismatch that dmon-core's `fix-meai-abi-pin` addresses. Mitigation: pin/align the transitive M.E.AI version to 10.5.1 (the version `Dmon.Abstractions` currently pins; re-check if dmon-core bumps it).
