...
- **Version alignment**: `ModelContextProtocol` 1.3.0 requires `Microsoft.Extensions.AI.Abstractions` **≥ 10.5.2**, so dmon-meko pins **10.5.2** (one patch above dmon-core's `Microsoft.Extensions.AI` 10.5.1). Both ship `AssemblyVersion 10.5.0.0`, so they are ABI-compatible at runtime; keep both within the same `10.5.x` minor.
