...
- **[assumed → verify on Discord]** Accepted values for Meko's `scope` and the mapping from mem0's id-filter model. *Default:* `MemoryScope { Session, Agent, User, Shared }`, default `Agent` (see D9).
