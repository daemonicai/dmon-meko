...
The dmon long-term memory tier: the Meko-over-MCP `ILongTermMemory` implementation, opt-in capture, the scope model's long-term binding, result provenance/relations, the long-term consistency/flush contract, and the disabled/no-op path. (The short-term tier and the cross-tier facade are specified and built elsewhere; this capability covers only the long-term requirements.)
